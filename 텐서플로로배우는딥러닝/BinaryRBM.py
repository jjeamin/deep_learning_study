...
def binary(probs):
    return tf.floor(probs + tf.random_uniform(tf.shape(probs),0.1)) # 균등분포

# Gibbs 표본추출
def cd_step(x_k):
    h_k = binary(tf.sigmoid(tf.matmul(x_k,W) + b_h))                # x -> h
    x_k = binary(tf.sigmoid(tf.matmul(h_k,tf.transpose(W)) + b_i))  # h -> x
    return x_k
...
# CD-2 알고리즘
# 1. 깁스 표본추출을 통해 새로운 입력값 x_s를 구한다.
# 2. 새로운 x_s 기반으로 새로운 은닉노드값 act_h_s를 구한다.
x_s = cd_gibbs(2,x)
act_h_s = tf.sigmoid(tf.matmul(x_s,W) + b_h)                        # 생성된 x -> h
# 입력값이 주어질 때 은닉노드값 act_h를 구한다
act_h = tf.sigmoid(tf.matmul(x,W) + b_h)                            # 학습 x -> h
# 은닉노드값이 주어질 때 입력값을 추출함
